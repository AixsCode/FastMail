...
public class RecyclerViewpachAdapter extends RecyclerView.Adapter<RecyclerViewpachAdapter.ViewHolder> implements View.OnClickListener {
    ...
    public RecyclerViewpachAdapter(Context mContext) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.recycler_patch_item, parent, false);
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
       ...
       // holder.setRandomIconByGlide(Application.getContextObject(), R.id.order_label);
        if(position==0) {
            holder.orderLabel.setImageBitmap(Util_GetBitmapFromResource.getBitmapFromResource(Application.getContextObject(),R.drawable.order_send_label));
        }
        else  if(position==1) {
            holder.orderLabel.setImageBitmap(Util_GetBitmapFromResource.getBitmapFromResource(Application.getContextObject(),R.drawable.order_post_label));
        }
        else if(position==3)
        {
            holder.orderLabel.setImageBitmap(Util_GetBitmapFromResource.getBitmapFromResource(Application.getContextObject(), R.drawable.order_store_label));
        }
        else
        {
            holder.setRandomIconByGlide(Application.getContextObject(), R.id.order_label);
